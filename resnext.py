...
def ResNextImageNet(input_shape=None, depth=[3, 4, 6, 3], cardinality=32, width=4, weight_decay=5e-4,
                    include_top=True, weights=None, input_tensor=None,
                    pooling=None, classes=1000, pre_weights=None):
...
def __initial_conv_block_imagenet(input, weight_decay=5e-4):
    ...
    x = Conv2D(64, (7, 7), padding='same', use_bias=False, strides=(2, 2))(input)
    x = BatchNormalization(axis=channel_axis)(x)
    x = Activation('relu')(x)
...
def __grouped_convolution_block(input, grouped_channels, cardinality, strides, weight_decay=5e-4):
    ...
    if cardinality == 1:
        ...
        x = Conv2D(grouped_channels, (3, 3), padding='same', use_bias=False, strides=(strides, strides))(init)
        x = BatchNormalization(axis=channel_axis)(x)
        x = Activation('relu')(x)
        return x
    ...
    for c in range(cardinality):
        ...
        x = Conv2D(grouped_channels, (3, 3), padding='same', use_bias=False, strides=(strides, strides))(x)
...
def __bottleneck_block(input, filters=64, cardinality=8, strides=1, weight_decay=5e-4):
    ...
    if K.image_data_format() == 'channels_first':
        if init._keras_shape[1] != 2 * filters:
            init = Conv2D(filters * 2, (1, 1), padding='same', strides=(strides, strides))(init)
    ...
    x = Conv2D(filters, (1, 1), padding='same', use_bias=False)(input)
    x = BatchNormalization(axis=channel_axis)(x)
    x = Activation('relu')(x)
...
def __create_res_next(nb_classes, img_input, include_top, depth=29, cardinality=8, width=4,
    ...
    if include_top:
        ...
        x = Dense(nb_classes, use_bias=False, activation='softmax')(x)
    else:
        if pooling == 'avg':
            x = GlobalAveragePooling2D()(x)
        elif pooling == 'max':
...
if __name__ == '__main__':
    model = ResNext((512, 512, 3), depth=29, cardinality=8, width=64)
    model.summary()